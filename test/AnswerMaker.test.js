...
const AnswerMaker = artifacts.require('AnswerMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('AnswerMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.Answer = await AnswerToken.new({ from: alice });
        await this.Answer.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await AnswerMaker.new(this.factory.address, bar, this.Answer.address, this.weth.address);
        this.AnswerWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.Answer.address)).logs[0].args.pair);
    ...
    it('should make Answers successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.AnswerWETH.address, '10000000', { from: minter });
        await this.Answer.transfer(this.AnswerWETH.address, '10000000', { from: minter });
        await this.AnswerWETH.mint(minter);
        ...
        // After calling convert, bar should have Answer value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.Answer.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for Answer-ETH pair
        await this.Answer.transfer(this.AnswerWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.AnswerWETH.address, '100000', { from: minter });
        await this.AnswerWETH.sync();
        await this.Answer.transfer(this.AnswerWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.AnswerWETH.address, '10000000', { from: minter });
        await this.AnswerWETH.mint(minter);
        assert.equal((await this.AnswerWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.Answer.address, this.weth.address);
        assert.equal((await this.Answer.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.AnswerWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});